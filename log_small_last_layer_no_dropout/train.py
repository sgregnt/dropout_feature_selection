...
def evaluate_mask():
    with tf.Graph().as_default():
        ...
        class_acc_res = []
        k = 50
        for i in range(k, FEATURE_SIZE, k):
            log_string('**** feature %03d removed ****' % (i))
            sys.stdout.flush()
            mask = np.ones(shape=(BATCH_SIZE, FEATURE_SIZE))
            mask[:, i-k:i] = 0
            mean_loss, acc, class_acc = eval_one_epoch(sess, ops, test_writer, mask)
            mean_loss_res.append(mean_loss)
            acc_res.append(acc)
            class_acc_res.append(class_acc)

    plt.figure()
    plt.plot(mean_loss_res)
    plt.xlabel('neuron')
    plt.ylabel('mean loss')
    plt.title('loss for a removed range of neurons')
